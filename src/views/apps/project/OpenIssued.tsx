...
const OpenIssued = ({ projectdetail }: Props) => {
    return (

        <Card>
            <CardHeader title='Open' subheader={`${projectdetail?.total_issue_open} issues`} />
            <CardContent>
                <Box
                    sx={{
                        overflowY: 'auto',
                    ...
                    {projectdetail?.issue.opened.map((issuedetail: Opened, index: number) => (
                        <Box
                            key={issuedetail.title}
                            sx={{
                                display: 'flex',
                                alignItems: 'center',
                                mb: index !== projectdetail?.issue.opened.length - 1 ? 7 : undefined
                            }}
                        >
