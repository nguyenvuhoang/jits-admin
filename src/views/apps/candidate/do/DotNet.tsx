...
type Props = {
    ...
    errors: any
    control: any
    setIsDotNetValid: any
}
type SelectedValues = {
    [key: string]: string | undefined;
};
const DotNet = ({ question, control, errors, setIsDotNetValid }: Props) => {
    ...
    const watchedDotNetValues = useWatch({control});

    useEffect(() => {

        if (Object.keys(watchedDotNetValues).length !== 0) {
            let hasKey = false;
            for (const key in watchedDotNetValues) {
            ...
            if (hasKey) {
                const allDotNetValuesSelected = Object.values(watchedDotNetValues).every(value => value !== undefined);
                setIsDotNetValid(allDotNetValuesSelected);
            }
        }
